...
{
	return [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithBool:NO],
													  NSFullScreenModeAllScreens,
													  [NSNumber numberWithInt:NSApplicationPresentationHideDock |
...
CursorClipper::CursorClipper( const NSRect& clipScreenRect ) : m_clipRect( clipScreenRect )
{
...
NSEvent* CursorClipper::ProcessEvent( NSEvent* event )
{
	...
	m_mouseLocation.x =
		std::min( std::max( m_mouseLocation.x, m_clipRect.origin.x ), m_clipRect.origin.x + m_clipRect.size.width );
	m_mouseLocation.y =
		std::min( std::max( m_mouseLocation.y, m_clipRect.origin.y ), m_clipRect.origin.y + m_clipRect.size.height );

	auto currentLocation = [NSEvent mouseLocation];

	CGWarpMouseCursorPosition(
		CGPointMake( m_mouseLocation.x, NSHeight( NSScreen.screens[0].frame ) - m_mouseLocation.y ) );

	WarpRec warpRec = { m_mouseLocation.x - currentLocation.x,
						m_mouseLocation.y - currentLocation.y,
...
- (void)sendEvent:(NSEvent*)event
{
	...
	else if( [event type] == NSEventTypeKeyDown && event.keyCode == kVK_Tab &&
...
- (void)updateTrackingAreas
{
	...
	m_trackingArea = [[NSTrackingArea alloc] initWithRect:[self bounds]
												  options:( NSTrackingCursorUpdate | NSTrackingMouseMoved |
...
void Tr2MainWindow::OnSetMarkedTextIME_MacOS( const std::wstring& string,
											  uint64_t selectedLocation,
...
Tr2MainWindow::Rect Tr2MainWindow::GetDesktopRect() const
{
	...
	return { int32_t( desktop.origin.x ),
...
{
	auto size = [NSWindow frameRectForContentRect:NSMakeRect( 0, 0, clientSize.width, clientSize.height )
										styleMask:GetWindowStyleMask( mode )];
...
{
	auto size = [NSWindow contentRectForFrameRect:NSMakeRect( 0, 0, windowSize.width, windowSize.height )
										styleMask:GetWindowStyleMask( mode )];
	return { uint32_t( size.size.width ), uint32_t( size.size.height ) };
}
...
void Tr2MainWindow::OnWindowDidChangeBackingProperties_MacOS()
{
	...
	{
		[window setContentSize:NSMakeSize( std::max( newMode.width, m_minimumSize.width ) * factor,
										   std::max( newMode.height, m_minimumSize.height ) * factor )];
	}
	else
	{
		SetState( false, newMode );
...
void Tr2MainWindow::SanitizeWindowPosition( Tr2MainWindowState::State& state ) const
{
	if( state.windowMode == Tr2WindowMode::FIXED_WINDOW )
	{
		...
		CCP_LOG(
			"Tr2MainWindow::SanitizeWindowPosition: screen frame position is (%f, %f), size (%f, %f), backing scale factor %f",
			frame.origin.x,
			frame.origin.y,
			frame.size.width,
			frame.size.height,
...
Tr2MainWindow::Size Tr2MainWindow::GetLargestWindowSize( uint32_t adapter, Tr2WindowMode::Type mode ) const
{
	if( [NSScreen screensHaveSeparateSpaces] )
	{
		...
		case Tr2WindowMode::WINDOWED:
			return { uint32_t( screen.visibleFrame.size.width * screen.backingScaleFactor ),
					 uint32_t( screen.visibleFrame.size.height * screen.backingScaleFactor ) };
		case Tr2WindowMode::FULL_SCREEN: {
			auto screenFrame = screen.visibleFrame;
			return { uint32_t( screenFrame.size.width * screen.backingScaleFactor ),
					 uint32_t( screenFrame.size.height * screen.backingScaleFactor ) };
		}
		default: {
