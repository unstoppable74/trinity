...
static PyObject* PyPick( PyObject* self, PyObject* args )
{
	...
	int x, y, components;
	if( !PyArg_ParseTuple( args, "iiOOOi", &x, &y, &pyProjection, &pyView, &pyViewport, &components ) )
	{
		return NULL;
	}
	...
	{
		PyObject* key = ToPython( ITr2PickableScene::PICK_OBJECT );
		PyObject* value;
		if( results.object )
		{
	...
	if( results.components & ITr2PickableScene::PICK_AREA )
	{
		...
		PyObject* key = ToPython( ITr2PickableScene::PICK_AREA );
	...
	{
		PyObject* key = ToPython( ITr2PickableScene::PICK_POSITION );
	...
	{
		PyObject* key = ToPython( ITr2PickableScene::PICK_UV );
