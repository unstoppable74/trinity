...
public:
	...
	template <typename T>
	void CopyToPSFloatBuffer( const T& objectRef )
	{
...
public:
	...
	template <typename T>
	void CopyToVSFloatBuffer( const T& objectRef )
	{
		// In-place buffer must be large enough to contain the data in the type being set
...
public:
	...
	float* GetSkinningMatrices() const
	{
		return m_data;
	}
	...
	void SetMirrorMatrix( const Matrix& mirrorMat )
	{
		m_mirrorMatrix = mirrorMat;
	}
	...
	{
		m_worldPos = Vector4( worldPos.x, worldPos.y, worldPos.z, 0.0f );
	}
...
public:
	...
	void SetPerObjectData( const Tr2PerObjectDataSkinned& perObjectData )
	{
		m_perObjectDataPtr = &perObjectData;
	};

	const unsigned GetJointCount() const
	{
		return m_jointCount;
	}
	const float* GetMatrices() const
	{
		return (const float*)m_jointTransforms;
	}
