...
// --------------------------------------------------------------------------------------
Tr2Vector2Parameter::Tr2Vector2Parameter( IRoot* lockobj ) :
	m_isUsedByEffect( false ),
	m_isSrgb( false ),
	m_value( 1.0f, 1.0f ),
	m_linearValue( 1.0f, 1.0f ),
	m_reroutedValue( NULL ),
	m_bindings( "Tr2Vector2Parameter/m_bindings" )
{
}
...
// --------------------------------------------------------------------------------------
const char* Tr2Vector2Parameter::GetParameterName() const
{
	return m_name.c_str();
}

unsigned Tr2Vector2Parameter::GetHashValue( unsigned startingHash ) const
{
...
//   size - Number of bytes to copy (could differ from the size of a vector2)
// --------------------------------------------------------------------------------------
void Tr2Vector2Parameter::CopyValueToEffect( Tr2RenderContextEnum::ShaderType inputType,
...
// --------------------------------------------------------------------------------------
void Tr2Vector2Parameter::SetValue( const Vector2& value )
{
	m_value = value;
...
// --------------------------------------------------------------------------------------
const Vector2& Tr2Vector2Parameter::GetValue( void )
{
	if( m_reroutedValue )
	{
...
// --------------------------------------------------------------------------------------
// Description:
//   Query whether the vector2 parameter is rerouted
// Return Value:
//   true, if the parameter is rerouted
//   false, otherwise
// --------------------------------------------------------------------------------------
bool Tr2Vector2Parameter::IsRerouted() const
{
...
// --------------------------------------------------------------------------------------
void Tr2Vector2Parameter::SetDestination( void* dest, size_t size )
{
	if( size >= sizeof( float ) * 2 && !m_isSrgb )
	{
...
// --------------------------------------------------------------------------------------
void Tr2Vector2Parameter::GetDestination( void*& dest, size_t& size )
{
	if( m_reroutedValue )
	{
...
//   vb - Value binding
// --------------------------------------------------------------------------------------
void Tr2Vector2Parameter::RegisterBinding( TriValueBinding* vb )
{
	CCP_ASSERT( vb );
...
// --------------------------------------------------------------------------------------
void Tr2Vector2Parameter::UnregisterBinding( TriValueBinding* vb )
{
	CCP_ASSERT( vb );
...
// --------------------------------------------------------------------------------------
// Description:
//   Utility function for getting X-component of Vector2.  This is used to setup a Blue
...
// --------------------------------------------------------------------------------------
float Tr2Vector2Parameter::GetX( void )
{
	if( m_reroutedValue )
	{
		m_value.x = m_reroutedValue[0];
...
// --------------------------------------------------------------------------------------
// Description:
//   Utility function for getting Y-component of Vector2.  This is used to setup a Blue
//   property.  If the value is rerouted, it refreshes the Vector3 X-component from the
//   rerouted buffer before returning the value.
// Return Value:
//   The Y-component of the Vector3
// --------------------------------------------------------------------------------------
float Tr2Vector2Parameter::GetY( void )
{
	if( m_reroutedValue )
	{
		m_value.y = m_reroutedValue[1];
...
// --------------------------------------------------------------------------------------
// Description:
//   Utility function for setting X-component of Vector2.  This is used to setup a Blue
//   property.  If the value is rerouted, the argument is assigned in the rerouted
//   buffer as well.
// Arguments:
//   x - The new X-component value
// --------------------------------------------------------------------------------------
void Tr2Vector2Parameter::SetX( float x )
{
	m_value.x = x;
	if( m_isSrgb )
	{
...
// --------------------------------------------------------------------------------------
// Description:
//   Utility function for setting Y-component of Vector2.  This is used to setup a Blue
//   property.  If the value is rerouted, the argument is assigned in the rerouted
//   buffer as well.
// Arguments:
//   y - The new Y-component value
// --------------------------------------------------------------------------------------
void Tr2Vector2Parameter::SetY( float y )
{
	m_value.y = y;
	if( m_isSrgb )
	{
...
bool Tr2Vector2Parameter::Initialize()
{
