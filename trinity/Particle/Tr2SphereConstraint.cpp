		...
		[=]( const Tr2BlockedRange<size_t>& range ) -> void {
			...
			for( auto i = range.begin(); i < range.end(); ++i, ++position, ++radiusStream, ++velocity )
			{
				...
				if( m_radiusElement.m_offset != -1 )
				{
					if( m_invertSphere )
					{
						radius = XMVectorSubtract( radius, XMVector4Dot( XMLoadFloat4( radiusStream ), particleRadiusCoefficient ) );
					}
					else
					{
						radius = XMVectorAdd( radius, XMVector4Dot( XMLoadFloat4( radiusStream ), particleRadiusCoefficient ) );
					}
				}
				...
				for( auto it = m_onCollisionEmitters.begin(); it != m_onCollisionEmitters.end(); ++it )
				{
					( *it )->SpawnParticles( arguments, reinterpret_cast<Vector3*>( position.Get() ), m_velocityElement.m_offset != -1 ? reinterpret_cast<Vector3*>( velocity.Get() ) : nullptr );
				}
			}
		} );
}
