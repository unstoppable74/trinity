...
std::function<void()> s_swapchainCreator = []() {};
std::function<void()> s_swapchainDestroyer = []() {};
...
{
	s_swapchainCreator = creator;
}

void SetSwapchainDestroyer( std::function<void()> destroyer )
{
	s_swapchainDestroyer = destroyer;
}
...
HRESULT SlCreateDXGIFactory2( UINT flags, REFIID id, void** factory2 )
{
	if( IsAvailable() )
	{
		...
		static auto slCreateDXGIFactory2 = reinterpret_cast<PFunCreateDXGIFactory2>( GetProcAddress( s_streamlineModule, "CreateDXGIFactory2" ) );
		return slCreateDXGIFactory2( flags, id, factory2 );
	}
	return CreateDXGIFactory2( flags, id, factory2 );
}
...
HRESULT SlD3D12CreateDevice( IUnknown* adapter, D3D_FEATURE_LEVEL featureLevel, REFIID id, void** device )
{
	if( IsAvailable() )
	{
		...
		static auto slD3D12CreateDevice = reinterpret_cast<PFunD3D12CreateDevice>( GetProcAddress( s_streamlineModule, "D3D12CreateDevice" ) );
		return slD3D12CreateDevice( adapter, featureLevel, id, device );
	}
	return D3D12CreateDevice( adapter, featureLevel, id, device );
}
...
void Toggle( StreamlineUtils::StreamlinePlugin plugin, bool enable )
{
	...
	if( plugin == StreamlineUtils::SP_DLSSG )
	{
		// special consideration is needed for dlssg which needs to destroy the swapchain, toggle and recreate the swapchain
		s_swapchainDestroyer();
...
void SetCommonConstants( StreamlineUtils::CommonConstants constants )
{
	...
	s_commonConstants.reset = constants.reset ? sl::eTrue : sl::eFalse;
...
void FreeResources()
{
	if( IsAvailable() )
	{
		...
		for( auto& pluginInfo : s_pluginInfo )
		{
			if( pluginInfo.second.created )
			{
				s_slFreeResources( pluginInfo.second.streamlineID, 0 );
			}
		}
	}
}
