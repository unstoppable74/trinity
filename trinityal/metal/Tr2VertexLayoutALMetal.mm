...
{
Tr2VertexLayoutAL::Tr2VertexLayoutAL() : m_renderContext( nullptr )
{
}
...
Tr2VertexLayoutAL::MetalVertexDescriptor
	Tr2VertexLayoutAL::GenerateVertexDescriptor( const std::vector<Tr2ShaderPipelineInputAL>& shaderInputs ) const
{
	MetalContext* metalContext = m_renderContext->GetMetalContext();
	MetalVertexDescriptor result = { metalContext->CreateVertexLayout(), 0, 0, false };

	static_assert( sizeof( result.streamMask ) * 8 >= METAL_VERTEX_STREAM_BUFFER_COUNT,
...
void Tr2VertexLayoutAL::SetVertexLayout( const std::shared_ptr<TrinityALImpl::Tr2ShaderProgramAL>& shaderProgram,
										 Tr2RenderContextAL& renderContext )
{
	...
	else if( isInParallelEncoding )
	{
		...
		descriptor.descriptor =
			queue->GetCachedVertexDescriptor( this, inputHash, descriptor.streamMask, descriptor.needsDummyStream );
		if( !descriptor.descriptor )
		{
			descriptor = GenerateVertexDescriptor( shaderProgram->GetInputs() );
			queue->CacheVertexDescriptor(
				this, inputHash, descriptor.descriptor, descriptor.streamMask, descriptor.needsDummyStream );
		}
	}
	else
	{
	...
	{
		renderContext.GetMetalWorkQueue()->SetVertexStream(
			METAL_VERTEX_STREAM_DUMMY, renderContext.GetMetalContext()->GetDummyBuffer(), sizeof( float ) * 4, 0 );
	}
	renderContext.GetMetalWorkQueue()->SetCurrentVertexDescriptor(
		descriptor.descriptor, descriptor.streamMask, descriptor.baseHash );
}

void Tr2VertexLayoutAL::AddVertexDescriptor( size_t inputHash,
