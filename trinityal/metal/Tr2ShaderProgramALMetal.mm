...
ALResult Tr2ShaderProgramAL::Create( ::Tr2ShaderAL* shaders, size_t count, Tr2PrimaryRenderContextAL& renderContext )
{
	...
	for( size_t i = 0; i < count; ++i )
	{
		...
		case VERTEX_SHADER:
			...
			m_vertexFunction =
				CompileShader( *shader, entryPointOverride ? entryPointOverride : @"mainVS", renderContext );
			if( !m_vertexFunction )
			{
		...
		case PIXEL_SHADER:
			...
			m_fragmentFunction =
				CompileShader( *shader, entryPointOverride ? entryPointOverride : @"mainPS", renderContext );
			if( !m_fragmentFunction )
			{
		...
		case COMPUTE_SHADER:
			...
			m_computeFunction =
				CompileShader( *shader, entryPointOverride ? entryPointOverride : @"mainCS", renderContext );
			if( !m_computeFunction )
			{
...
id<MTLFunction> Tr2ShaderProgramAL::CompileShader( const ::Tr2ShaderAL& shader,
												   NSString* entryFunction,
												   Tr2PrimaryRenderContextAL& renderContext )
{
	id<MTLDevice> mtlDevice = m_metalContext->GetDevice();

	dispatch_data_t shaderData = dispatch_data_create( shader.m_shader->m_bytecode.get(),
