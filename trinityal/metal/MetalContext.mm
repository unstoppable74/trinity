...
CCP_STATS_DECLARE( constantDataSize,
				   "Trinity/AL/constantDataSize",
				   true,
				   CST_MEMORY,
				   "Total size of constant data uploaded per frame" );
...
id<MTLBuffer> MetalContext::CreateMetalBuffer( MetalWorkQueue* workQueue,
											   size_t sizeInBytes,
...
void MetalContext::GenerateDummyTexture()
{
	...
	for( uint32_t i = 0; i < METAL_NUM_DUMMY_TEXTURES; ++i )
	{
		...
		m_dummyTexture[i] = CreateMetalTexture( dummyTextureProperties[i].type,
...
void MetalContext::GenerateDummyBuffer()
{
	...
	m_dummyBuffer =
		CreateMetalBuffer( &m_primaryWorkQueue, sizeof( bufferData ), MTLResourceStorageModePrivate, bufferData );
}
...
id<MTLTexture> MetalContext::CreateUAVOfMetalTexture( id<MTLTexture> texture, uint32_t mipLevel )
{
	...
	case MTLTextureTypeCubeArray:
		return [texture newTextureViewWithPixelFormat:texture.pixelFormat
										  textureType:MTLTextureType2DArray
											   levels:NSMakeRange( mipLevel, 1 )
											   slices:NSMakeRange( 0, 6 * texture.arrayLength )];
	default:
		if( mipLevel > 0 )
		{
			return [texture newTextureViewWithPixelFormat:texture.pixelFormat
...
void MetalContext::AddRenderPipelineStateToCache( size_t renderPipelineDescriptorHash,
...
void MetalContext::AddComputePipelineStateToCache( size_t computePipelineDescriptorHash,
...
{
	m_pendingRelease.erase( std::remove_if( begin( m_pendingRelease ),
											end( m_pendingRelease ),
											[&]( auto& item ) { return item.frame <= renderedFrame; } ),
							end( m_pendingRelease ) );
}
