...
ALResult Tr2TextureAL::Create( const Tr2BitmapDimensions& desc,
							   ...
							   Tr2PrimaryRenderContextAL& renderContext )
{
	...
	MTLTextureType metalTextureType =
		metalContext->m_utils->GetMTLTextureType( desc.GetType(), desc.GetArraySize(), msaa.samples );
...

	{
		...
		m_mtlTexture = metalContext->CreateMetalTexture(
			metalTextureType,
			metalPixelFormat,
			desc.GetWidth(),
			desc.GetHeight(),
		...
		if( initialData )
		{
			...
			for( uint32_t slice = 0; slice < sliceCount; ++slice )
			{
				for( uint32_t mip = 0; mip < mipMapCount; ++mip )
				{
					...
					{
						workQueue->UploadTexture( m_mtlTexture,
												  initialData[index].m_sysMem,
												  slice,
												  mip,
												  initialData[index].m_sysMemPitch,
												  initialData[index].m_sysMemSlicePitch,
												  0,
												  false );
					}
					++index;
...
ALResult
	Tr2TextureAL::OpenShared( uintptr_t handle, Tr2GpuUsage::Type gpuUsage, Tr2PrimaryRenderContextAL& renderContext )
{
	return E_FAIL;
}
...
ALResult Tr2TextureAL::MapForWriting( const Tr2TextureSubresource& region,
									  void*& data,
									  uint32_t& pitch,
									  Tr2RenderContextAL& renderContext )
{
...
void Tr2TextureAL::UnmapForWriting( Tr2RenderContextAL& renderContext )
{
	...
	{
		MTLOrigin origin =
			MTLOriginMake( m_mappedRegion.m_box.left, m_mappedRegion.m_box.top, m_mappedRegion.m_box.front );
		...
		renderContext.GetMetalWorkQueue()->UploadTexture( m_mtlTexture,
														  m_mtlWriteBuffer,
														  m_mappedRegion.m_startFace,
														  m_mappedRegion.m_startMipLevel,
														  origin,
														  size,
														  m_dataLayout.m_sysMemPitch,
														  m_dataLayout.m_sysMemSlicePitch,
														  m_mappedRange.offset,
														  false );
	}
	else
	{
...
ALResult Tr2TextureAL::UpdateSubresource( const Tr2TextureSubresource& region,
										  const void* source,
										  uint32_t pitch,
										  uint32_t slicePitch,
										  Tr2RenderContextAL& renderContext )
...
ALResult Tr2TextureAL::Resolve( Tr2TextureAL& destination, Tr2RenderContextAL& renderContext )
{
	if( m_msaa.samples <= 1 )
	{
		return destination.CopySubresourceRegion(
			Tr2TextureSubresource(), *this, Tr2TextureSubresource(), renderContext );
	}
...
void Tr2TextureAL::Describe( Tr2DeviceResourceDescriptionAL& description ) const
{
	...
	description["size"] =
		std::to_string( size * std::max( 1u, m_desc.GetArraySize() ) * std::max( 1u, m_msaa.samples ) );
