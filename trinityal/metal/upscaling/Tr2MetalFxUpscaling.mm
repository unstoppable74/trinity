...
template <typename Item, typename Processor> class WorkQueue
{
public:
	WorkQueue( Processor processor ) : m_processor( processor )
	{
...
Tr2MetalFxUpscalingTechnique::Tr2MetalFxUpscalingTechnique( Tr2RenderContextAL& renderContext,
															Tr2UpscalingAL::Technique technique,
															Tr2UpscalingAL::Setting setting,
															bool frameGeneration,
															uint32_t adapter ) :
	Tr2UpscalingTechniqueAL( renderContext, technique, setting, frameGeneration, adapter ), m_temporal( false )
{
...
Tr2UpscalingContextAL*
	Tr2MetalFxUpscalingTechnique::CreateContextInstance( Tr2UpscalingAL::UpscalingContextParams params )
{
...
std::vector<Tr2UpscalingAL::Setting> Tr2MetalFxUpscalingTechnique::GetAvailableSettings() const
{
	if( m_temporal )
	{
		return { Tr2UpscalingAL::Setting::ULTRA_QUALITY,
	...
	return { Tr2UpscalingAL::Setting::QUALITY,
			 Tr2UpscalingAL::Setting::BALANCED,
			 Tr2UpscalingAL::Setting::PERFORMANCE,
			 Tr2UpscalingAL::Setting::ULTRA_PERFORMANCE };
}

Tr2MetalFxUpscalingContext::Tr2MetalFxUpscalingContext( Tr2UpscalingAL::Setting setting,
														bool temporal,
														Tr2UpscalingAL::UpscalingContextParams params ) :
	Tr2UpscalingContextAL( setting, false, params ), m_setup( false ), m_renderContext( params.renderContext )
{
	if( @available( macOS 13.0, * ) )
	{
...
void Tr2MetalFxUpscalingContext::CreateTemporalScaler()
{
	if( @available( macOS 13.0, * ) )
	{
		static WorkQueue<TemporalScaler, void ( * )( const std::shared_ptr<TemporalScaler>& )> s_queue(
			[]( auto scaler ) {
				if( !scaler->canceled )
				{
		...
		auto depthPixelFormat = metalContext->m_utils->GetMTLPixelFormat(
			Tr2RenderContextEnum::ConvertDepthStencilFormat( m_params.depthFormat ) );
		// motion pixel format is always the same, true as of April 2024
		auto motionPixelFormat =
			metalContext->m_utils->GetMTLPixelFormat( Tr2RenderContextEnum::PIXEL_FORMAT_R16G16_FLOAT );
