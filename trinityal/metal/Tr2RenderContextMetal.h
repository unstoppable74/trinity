...
public:
	...
	ALResult ClearUav( const Tr2TextureAL& texture, uint32_t mipLevel, const float values[4] ) throw();
	ALResult ClearUav( const Tr2TextureAL& texture, uint32_t mipLevel, const uint32_t values[4] ) throw();

	ALResult
		CopySubBuffer( Tr2BufferAL& dest, uint32_t destOffset, Tr2BufferAL& src, uint32_t offset, uint32_t length );
	...
	ALResult DrawIndexedInstanced( uint32_t indexCountPerInstance,
								   uint32_t instanceCount,
								   uint32_t startIndexLocation,
								   int32_t baseVertexLocation,
								   uint32_t startInstanceLocation );
	ALResult DrawInstanced( uint32_t vertexCountPerInstance,
							uint32_t instanceCount,
							uint32_t startVertexLocation,
							uint32_t startInstanceLocation );

	ALResult DrawIndexedPrimitiveUP( uint32_t numVertices,
									 uint32_t primitiveCount,
	...
	ALResult
		DrawPrimitiveUP( uint32_t primitiveCount, const void* vertexStreamZeroData, uint32_t vertexStreamZeroStride );
	...
	ALResult DispatchRays( Tr2RtPipelineStateAL& pipeline,
	...
	ALResult SetConstants( const Tr2ConstantBufferAL& buffer,
	...
	ALResult Clear( uint32_t clearFlags, uint32_t color, float depth, uint32_t stencil = 0, uint32_t slot = 0 );
	...
	void
		RenderPassHint( const Tr2ColorAttachment& rt0, const Tr2ColorAttachment& rt1, const Tr2DepthAttachment& depth );
	...
	ALResult GetRenderTargetSize( uint32_t& width, uint32_t& height, uint32_t slot = 0 );
	...
	static const uint32_t SHADER_TYPE_MASK = ( 1 << Tr2RenderContextEnum::VERTEX_SHADER ) |
		( 1 << Tr2RenderContextEnum::PIXEL_SHADER ) | ( 1 << Tr2RenderContextEnum::COMPUTE_SHADER );

	// Debug helpers
	size_t GetStackSizeRT( uint32_t RT = 0 ) const
	{
		return 0;
	}
	...
	ALResult GetGpuPageFaultResource( Tr2RenderContextEnum::PixelFormat& format,
									  uint64_t& size,
									  uint32_t& width,
									  uint32_t& height,
									  uint32_t& depth,
	...
	TrinityALImpl::MetalContext* GetMetalContext()
	{
		return m_metalContext;
	}
	TrinityALImpl::MetalWorkQueue* GetMetalWorkQueue()
	{
		return m_workQueue;
	}
	Tr2PresentParametersAL* GetPresentParamaters()
	{
	...
	ALResult UseResources( Tr2UseResourceDestination dest,
	...
	Tr2UpscalingAL::Result EnableUpscaling( Tr2UpscalingAL::Technique tech,
											Tr2UpscalingAL::Setting setting,
											bool frameGeneration,
											uint32_t adapter );
	Tr2UpscalingContextAL* GetUpscalingContext( uint32_t upscalingContextID ) const;
	Tr2UpscalingContextAL* CreateUpscalingContext( Tr2UpscalingAL::UpscalingContextParams params,
												   uint32_t existingContext = Tr2UpscalingAL::INVALID_CONTEXT_ID );
	...
	void GetUpscalingSetup( Tr2UpscalingAL::Technique& technique,
							Tr2UpscalingAL::Setting& setting,
							bool& framegeneration,
							bool& temporal ) const;
	std::vector<std::tuple<Tr2UpscalingAL::Technique, uint32_t, bool>>
		GetSupportedUpscalingTechniques( uint32_t adapter );
