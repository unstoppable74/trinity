...
{
Tr2ConstantBufferAL::Tr2ConstantBufferAL() : m_metalContext( nullptr ), m_buffer( nullptr ), m_size( 0 )
{
}
...
ALResult Tr2ConstantBufferAL::Create( uint32_t size,
									  Tr2ConstantUsageAL::Type usage,
...
ALResult Tr2ConstantBufferAL::SetConstants( Tr2RenderContextEnum::ShaderType shaderType,
											uint32_t constantIndex,
											Tr2RenderContextAL& renderContext )
{
	if( m_buffer )
	{
		renderContext.GetMetalWorkQueue()->SetConstants(
			shaderType, m_buffer, m_size, m_token, METAL_CONST_BUFFER_OFFSET + constantIndex );
	}
