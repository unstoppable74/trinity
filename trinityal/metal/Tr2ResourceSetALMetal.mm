...
static_assert( sizeof( TrinityALImpl::ShaderResourceMask::constantBufferMask ) * 8 >= METAL_MAX_BOUND_BUFFERS,
			   "Please use a type with more bits for ShaderResourceMask::constantBufferMask." );
static_assert( sizeof( TrinityALImpl::ShaderResourceMask::bufferMask ) * 8 >= METAL_MAX_BOUND_BUFFERS,
...
static_assert( sizeof( TrinityALImpl::ShaderResourceMask::samplerMask ) * 8 >= METAL_MAX_BOUND_SAMPLERS,
			   "Please use a type with more bits for ShaderResourceMask::samplerMask." );


namespace TrinityALImpl
{
Tr2ResourceSetAL::Tr2ResourceSetAL() : m_isValid( false )
{
	static_assert( sizeof( m_buffersMask ) * 8 >= METAL_MAX_BOUND_BUFFERS,
...
ALResult Tr2ResourceSetAL::Create( const Tr2ResourceSetDescriptionAL& description,
								   const ::Tr2RtPipelineStateAL& pipeline,
								   Tr2PrimaryRenderContextAL& renderContext )
{
	...
	const ::Tr2ShaderProgramAL program = pipeline.m_pipeline->GetShaderProgram( 0 );
								   ...
								   Tr2PrimaryRenderContextAL& renderContext )
{
	...
	for( auto stage : stages )
	{
		...
		for( int i = 0; i < Tr2ResourceSetDescriptionAL::MAX_RESOURCES_IN_STAGE; ++i )
		{
			...
			const Tr2ResourceSetDescriptionAL::Resource& resource =
				description.m_srv[description.m_registerMap.srvs[stage][i]];
		...
		for( int i = 0; i < Tr2ResourceSetDescriptionAL::MAX_RESOURCES_IN_STAGE; ++i )
		{
			...
			const Tr2ResourceSetDescriptionAL::Resource& resource =
				description.m_uav[description.m_registerMap.uavs[stage][i]];
		...
		for( int i = 0; i < Tr2ResourceSetDescriptionAL::MAX_RESOURCES_IN_STAGE; ++i )
		{
			...
			const Tr2ResourceSetDescriptionAL::Sampler& sampler =
				description.m_samplers[description.m_registerMap.samplers[stage][i]];
		...
		while( textureMask )
		{
			if( textureMask & 0x1 )
			{
				m_textures[stage][index] = metalContext->GetDummyTexture(
					MTLTextureType( program.m_program->m_resourceMask[stage].textureTypes[index] ) );
		...
		m_texturesRange[stage] = ( texturesMin != NSUIntegerMax ) ?
			NSMakeRange( texturesMin, texturesMax - texturesMin + 1 ) :
			NSMakeRange( 0, 0 );
		m_samplersRange[stage] = ( samplersMin != NSUIntegerMax ) ?
